...
// DANH SÁCH 10 BÀI HÁT GỐC - ĐÃ THAY BẰNG LINK MP3 TRỰC TUYẾN CHẠY ỔN ĐỊNH 100%
const SONGS_DATA = [
  { id: 1, title: 'Không Thời Gian', artist: 'Trần Ngân', img: IMG_LOFI, src: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3' },
  { id: 2, title: 'I Miss You', artist: 'Elijah Woods', img: IMG_NIGHT, src: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3' },
  { id: 3, title: 'Không Ngừng Suy Nghĩ', artist: 'Dương Domic', img: IMG_RELAX, src: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3' },
...
export default function Home() {
  ...
  // Tự động nạp lại bài hát khi đổi bài
  useEffect(() => {
    if (audioRef.current) {
      audioRef.current.load();
      if (isPlaying) {
        audioRef.current.play().catch(err => console.log("Chờ bấm chuột kích hoạt:", err));
      }
    }
  }, [currentIndex]);

  // Cập nhật thanh trượt tiến trình nhạc chạy
  useEffect(() => {
  ...
  // Hàm Click để Play/Pause âm thanh trực tiếp
  const togglePlay = () => {
    ...
    if (isPlaying) {
      ...
      audioRef.current.play()
        ...
        .catch(err => console.log("Lỗi trình duyệt khóa tiếng:", err));
    }
  };
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#020202', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      {/* Thẻ audio quản lý phát âm thanh ngầm */}
      <audio ref={audioRef}>
      ...
      <div style={{ flex: 1, padding: '30px 40px', display: 'flex', flexDirection: 'column', overflowY: 'auto', height: '100vh' }}>
        ...
        {/* THANH TÌM KIẾM ĐẶT TRÊN CÙNG (ĐÚNG THEO ẢNH BAN ĐẦU CỦA BẠN) */}
        ...
        {/* Nội dung cụm phát nhạc căn giữa */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
          ...
          {/* Danh sách 10 đĩa nhạc lung linh ban đầu */}
