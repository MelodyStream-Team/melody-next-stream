...
// DANH SÁCH 3 BÀI HÁT TỪ VIDEO CỦA BẠN (Đã được chuyển đổi sang file âm thanh thực tế)
const SONGS_DATA = [
  { 
    id: 1, 
...
export default function Home() {
  ...
  // Cập nhật nguồn âm thanh sạch mỗi khi đổi bài
  useEffect(() => {
    if (audioRef.current) {
      audioRef.current.load();
      if (isPlaying) {
        audioRef.current.play().catch(err => console.log("Chặn tự động phát tiếng:", err));
      }
    }
  }, [currentIndex]);

  // Đồng bộ thời gian chạy của thanh trượt tua nhạc
  useEffect(() => {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#020202', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      {/* 1. THANH CHỨC NĂNG SIDEBAR BÊN TRÁI */}
      <div style={{ width: '240px', backgroundColor: '#0d0d0d', borderRight: '1px solid #1a1a1a', padding: '20px 0', display: 'flex', flexDirection: 'column', flexShrink: 0, position: 'sticky', top: 0, height: '100vh' }}>
        <div style={{ padding: '0 25px', marginBottom: '25px' }}>
          <h1 style={{ fontSize: '1.4rem', fontWeight: 'bold', margin: 0, color: '#ff4757' }}>🎵 Melody STREAM</h1>
          <p style={{ margin: '4px 0 0 0', fontSize: '0.65rem', color: '#666', textTransform: 'uppercase' }}>Ứng dụng nghe nhạc trực tuyến</p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          <div style={{ padding: '10px 25px', display: 'flex', alignItems: 'center', gap: '12px', color: '#fff', backgroundColor: '#111', borderLeft: '3px solid #ff4757', fontSize: '0.9rem', fontWeight: 'bold' }}>📻 <span>Khám Phá</span></div>
          <div style={{ padding: '10px 25px', display: 'flex', alignItems: 'center', gap: '12px', color: '#aaa', fontSize: '0.9rem' }}>📈 <span>#melodychart</span></div>
          <div style={{ padding: '10px 25px', display: 'flex', alignItems: 'center', gap: '12px', color: '#aaa', fontSize: '0.9rem' }}>🎵 <span>Thư Viện</span></div>
        </div>
      </div>

      {/* 2. NỘI DUNG CHÍNH TRÌNH PHÁT BÊN PHẢI */}
      <div style={{ flex: 1, padding: '40px', display: 'flex', flexDirection: 'column', alignItems: 'center', overflowY: 'auto', height: '100vh' }}>
        
        {/* Khung trình phát nhạc trung tâm */}
        ...
        {/* Danh sách Playlist 3 bài hát từ video của bạn */}
