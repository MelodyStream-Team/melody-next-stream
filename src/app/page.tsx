...
const IMG_SUMMER = 'https://images.unsplash.com/photo-1507524428034-b723cf961d3e?w=500&q=80';

// GIỮ NGUYÊN BẢN GỐC - CHỈ PHÁT ÂM THANH NGẦM TỪ 3 VIDEO CỦA BẠN (KHÔNG HIỆN VIDEO)
const SONGS_DATA = [
...
export default function Home() {
  ...
  // Tải lại nguồn âm thanh ngầm khi đổi bài
  useEffect(() => {
    if (audioRef.current) {
      audioRef.current.load();
      if (isPlaying) {
        audioRef.current.play().catch(err => console.log("Trình duyệt chặn tự động phát tiếng:", err));
      }
    }
  }, [currentIndex]);

  // Cập nhật thanh trượt thời gian thời gian nhạc chạy
  useEffect(() => {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#020202', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      {/* Thẻ audio ẩn hình hoàn toàn, chỉ chịu trách nhiệm phát tiếng */}
      <audio ref={audioRef}>
        <source src={currentSong.src} type="audio/mpeg" />
      </audio>

      {/* 1. THANH CHỨC NĂNG SIDEBAR GỐC BÊN TRÁI */}
      ...
      {/* 2. KHU VỰC PHÁT NHẠC GỐC BÊN PHẢI */}
      <div style={{ flex: 1, padding: '40px', display: 'flex', flexDirection: 'column', alignItems: 'center', overflowY: 'auto', height: '100vh' }}>
        ...
        <div style={{ marginBottom: '35px', width: '100%', maxWidth: '380px' }}>
          <input type="text" placeholder="🔍 Tìm kiếm bài hát hoặc ca sĩ..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ width: '100%', padding: '12px 20px', borderRadius: '25px', border: '1px solid #222', backgroundColor: '#0a0a0a', color: '#fff', textAlign: 'center', outline: 'none', fontSize: '0.9rem' }} />
        </div>

        <h3 style={{ fontSize: '1.2rem', marginBottom: '20px', color: '#666', letterSpacing: '1px', textTransform: 'uppercase' }}>Popular Songs</h3>

        {/* DANH SÁCH 10 ĐĨA NHẠC NGUYÊN BẢN BAN ĐẦU */}
